...
MODEL = os.environ.get("QWEN_MODEL", "Qwen2.5-14B-Instruct")
...
def build_listing_payload(df: pd.DataFrame, c: dict):
...
def answer(user_q: str, c: dict, items: list[dict]):
    return chat(
        [
...
def main():
    ...
    if not user_q:
        print('Usage: python rent_chatbot_end2end.py "your query"')
        sys.exit(1)

    c = normalize_constraints(llm_extract(user_q))
    k = int(c.get("k", 5) or 5)

    cand = faiss_recall(user_q, recall=200)
    filtered = apply_filters(cand, c)

    # fallback: drop must-have if too few
    ...
    items = build_listing_payload(filtered, c)

    print(answer(user_q, c, items))
